...
def main():
    """main"""
    val_n = count = int(input())
    singly_front = SinglyLinkedList()
    singly_last = SinglyLinkedList()
    singly_print = SinglyLinkedList()
    for _ in range(val_n):
        data = input()
        singly_front.insert_front(data)
        singly_last.insert_last(data)

    for i in range(1, val_n+1):
        if i%2 == 1:
            if count:
                singly_print.insert_last(singly_front.head.get_data())
                singly_front.delete(singly_front.head.get_data())
                count -= 1
            if count:
                singly_print.insert_last(singly_last.head.get_data())
                singly_last.delete(singly_last.head.get_data())
                count -= 1
        else:
            if count:
                singly_print.insert_last(singly_last.head.get_data())
